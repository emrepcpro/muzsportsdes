...
const SportsNews: React.FC<SportsNewsProps> = ({ aiEnabled = true, agingDays = 30 }) => {
  ...
  if (!loading && news.length === 0) return (
    <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
      <h2 className="text-xl font-bold mb-2">Haber bulunamadı</h2>
      <p className="text-white/40 mb-4">Dış servislerden veri alınamadı veya henüz içerik yok.</p>
      <div className="flex gap-3">
        <button onClick={() => { setLoading(true); setError(null); (async () => { const data = await dataService.getNews(); setNews(data); setLoading(false); })(); }} className="px-4 py-2 bg-primary text-black font-bold rounded-lg">Tekrar dene</button>
      </div>
  ...
  return (
    <div className="p-6 lg:p-10 max-w-7xl mx-auto">
      ...
      <div className="mt-16 bg-gradient-to-br from-primary/10 to-transparent border border-primary/20 rounded-3xl p-10 flex flex-col lg:flex-row items-center gap-10">
         <div className="flex-1 space-y-6"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center text-black"><Sparkles size={24} /></div><h3 className="text-2xl font-black italic tracking-tighter">AI PANELİ AKTİF</h3></div><p className="text-white/60 leading-relaxed">{aiEnabled ? `${agingDays} günü geçen tüm içerikler otomatik olarak AI tarafından özetlenir.` : 'AI özetleme şu an kapalı. İsterseniz manuel akışı açabilirsiniz.'}</p></div>
      </div>
    </div>
  );
};
